...
describe('ValidateResourcesIdsInterceptor', () => {
  ...
  it('should skip validation if the decorator is not present', async () => {
  ...
  it('should validate project id and throw if project is not found', async () => {
    ...
    await expect(interceptor.intercept(mockExecutionContext, mockCallHandler)).rejects.toThrow(NotFoundException)
  ...
  it('should validate both project and task continue if both exist', async () => {
    const mockRequest = {
      params: {
        projectId: 'project-1',
        taskId: 'task-1'
